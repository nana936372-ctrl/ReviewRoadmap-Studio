...
import type { Language, RawReview } from './src/domain/types';
import { SEMANTIC_ANALYSIS_SCHEMA } from './src/lib/analysis/semantic';
import { parseAppStorePageReviews } from './src/lib/appStorePageReviews';
...
function jsonResponse(response: any, statusCode: number, payload: unknown) {
...
function resolveLlmConfig(env: Record<string, string>): LlmConfig | null {
      ...
      server.middlewares.use(async (request, response, next) => {
        ...
        try {
          ...
          if (!appStoreResponse.ok) {
            response.statusCode = appStoreResponse.status;
            response.setHeader('Content-Type', 'application/json');
            response.end(JSON.stringify({ error: `App Store page returned ${appStoreResponse.status}.` }));
            return;
          }

          const html = await appStoreResponse.text();
          const reviews = parseAppStorePageReviews(html);

          response.statusCode = 200;
          response.setHeader('Content-Type', 'application/json');
          response.end(JSON.stringify({ feedUrl: appUrl, reviews }));
        } catch (error) {
    ...
    configureServer(server: any) {
      server.middlewares.use(async (request, response, next) => {
        ...
        if (request.method !== 'POST') {
          jsonResponse(response, 405, { error: 'Use POST for semantic analysis.' });
          return;
        }

        if (!llmConfig) {
          jsonResponse(response, 503, {
            error: 'Missing LLM API key. Add OPENAI_API_KEY or DEEPSEEK_API_KEY to .env.local.'
          });
          return;
        }
        ...
        try {
          ...
          if (reviews.length === 0) {
            jsonResponse(response, 400, { error: 'No review records supplied for semantic analysis.' });
            return;
          }
          ...
          if (!text) {
            jsonResponse(response, 502, { error: 'LLM response did not include JSON text.' });
            return;
          }

          jsonResponse(response, 200, { semanticAnalysis: parseJsonText(text) });
        } catch (error) {
          jsonResponse(response, 502, {
